#!/usr/bin/env node
/**
 * irida bin launcher: run compiled dist/cli.js and rebuild when src/ is newer.
 * Avoids a stale global `irida` after git pull without manual npm run build.
 */
...
if (distIsStale()) {
  process.stderr.write("irida: dist is stale — running npm run build…\n");
  if (!rebuild()) {
    process.stderr.write("irida: build failed — falling back to tsx (dev mode)\n");
    runViaTsx(process.argv.slice(2));
    process.stderr.write("irida: tsx fallback unavailable; try npm install && npm run build\n");
    process.exit(78);
  }
}
