...
3. Verify: `irida doctor` row `telegram allowed_updates`; `irida gateway status` same row.
...
- Prefer **irida gateway** for polling — it always passes `TELEGRAM_GATEWAY_ALLOWED_UPDATES`.
...
- Slash parser strips `@suffix` — use standard irida commands (`/status`, `/help`, …).

## Do not

- Revoke bot token in BotFather unless intentional — breaks all Telegram surfaces until `irida auth telegram login`.
