...
- `chat` and `tui` are interactive: destructive prompts require confirmation (`--yes-i-understand` skips it). **TUI:** virtual transcript scroll (↑↓, Ctrl+O/G/E), multiline composer, `@file`/`@dir` hints, grouped `/tools`, status bar run stats.
